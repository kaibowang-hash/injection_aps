...
def validate():
	...
	formal_production = frappe.db.sql(
		"""
		select
			coalesce(sum(case when se.custom_aps_output_type = 'Good' then sed.qty else 0 end), 0) as good_qty,
			coalesce(sum(case when se.custom_aps_output_type = 'Scrap' then sed.qty else 0 end), 0) as scrap_qty
		from `tabStock Entry` se
		inner join `tabStock Entry Detail` sed on sed.parent = se.name
		where se.docstatus = 1 and se.name like %(prefix)s and sed.is_finished_item = 1
		""",
	...
	checks = {
		...
		and frappe.db.count("APS Production Allocation", {"planning_run": run_name}) == 4,
		...
		"detail_traces": sum(len(row["production_allocations"]) for row in details) == 4
...
def _create_fixture():
	...
	if not all((company, workstation, item_group, stock_uom, customer_group, territory, warehouse)):
		frappe.throw(
			"Phase 4 UI fixture needs Company, Workstation, Item Group, UOM, Customer Group, Territory, and Warehouse records."
		)

	suffix = frappe.generate_hash(length=8).upper()
	customer = _create_customer(customer_group, territory, suffix)
	item = _create_item(item_group, stock_uom, suffix)
	work_order = _create_work_order(company, item, stock_uom, warehouse, suffix)
...
def _create_work_order(company, item, stock_uom, warehouse, suffix):

const SHIFT_PROPOSAL_SHARED_READY = frappe.require("/assets/injection_aps/js/injection_aps_ui_loader.js")
...
frappe.ui.form.on("APS Shift Schedule Proposal Batch", {
	async refresh(frm) {
		...
		await SHIFT_PROPOSAL_SHARED_READY;
...
function add_actions(frm) {
	...
	if (injection_aps.ui.can_run_action("review_shift_schedule_proposals") && pendingRows.length) {
		frm.add_custom_button(__("Approve Pending Rows", null, "Injection APS"), async () => {
			...
			const confirmed = await injection_aps.ui.confirm_action(
				{ action_key: "review_shift_schedule_proposals", confirm_required: 1 },
				{
					title: __("Confirm Shift Proposal Review", null, "Injection APS"),
					summary_lines: [
			...
			const response = await injection_aps.ui.xcall(
				{
					message: __("Approving pending shift proposal rows...", null, "Injection APS"),
					success_message: __("Pending shift proposal rows approved.", null, "Injection APS"),
					busy_key: `shift-proposal-review:${frm.doc.name}`,
				},
				"injection_aps.api.app.review_shift_schedule_proposals",
				{
